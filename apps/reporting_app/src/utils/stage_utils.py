...
class StageUtils:
    ...
    def calculate_tickets_duration_in_sprint(df: pd.DataFrame, sprint_name: str) -> pd.DataFrame:
        ...
        for stage in ALL_STAGE_COLUMNS_DURATIONS_IN_DAYS:
            start_col = StageUtils.to_stage_start_date_column_name(stage)
            end_col = StageUtils.to_stage_end_date_column_name(stage)
            days_col = StageUtils.to_stage_duration_days_column_name(stage)
        ...
        for stage in THRESHOLD_STAGE_COLUMNS_DURATION_IN_DAYS:
            start_col = StageUtils.to_stage_start_date_column_name(stage)
            end_col = StageUtils.to_stage_end_date_column_name(stage)
            sprint_days_col = StageUtils.to_stage_in_sprint_duration_days_column_name(stage)
    ...
    def to_stage_start_date_column_name(stage_series):
        ...
        return f"Stage {StageUtils.to_stage_name(stage_series)} start"

    @staticmethod
    def to_stage_end_date_column_name(stage_series):
        """Convert stage column names to clean stage names in stage end date."""
        return f"Stage {StageUtils.to_stage_name(stage_series)} end"
    ...
    def to_stage_duration_days_column_name(stage_series):
        ...
        return f"Stage {StageUtils.to_stage_name(stage_series)} days"

    @staticmethod
    def to_stage_in_sprint_duration_days_column_name(stage_series):
        """Convert stage column names to clean stage names in sprint duration days."""
        return f"Stage {StageUtils.to_stage_name(stage_series)} days in sprint"